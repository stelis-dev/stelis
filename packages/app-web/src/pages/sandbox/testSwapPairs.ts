 ...
 *
 * Indexed by network (testnet | mainnet) and by payment token TYPE so the
...
export interface TestSwapPair {
  ...
  baseType: string;
  quoteType: string;
  /** DeepBook swap function that converts SUI → paymentTokenType on this pool. */
...
/**
 * Look up a test swap pair by payment token type on the given network.
...
export function findTestSwapPair(
  ...
  return TEST_SWAP_PAIRS[network].find((p) => p.paymentTokenType === paymentTokenType) ?? null;
}
